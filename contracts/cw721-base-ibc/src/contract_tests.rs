...
fn minting() {
    ...
    let info = contract
        .nft_info(deps.as_ref(), token_id.clone(), class_id.clone())
        .unwrap();
    assert_eq!(
        info,
    ...
    assert_eq!(vec![(token_id, class_id)], tokens.tokens);
}
...
fn burning() {
    ...
    let burn_msg = ExecuteMsg::Burn { token_id, class_id };
...
fn approving_all_revoking_all() {
    ...
    assert_eq!(1, tokens.tokens.len());
    assert_eq!(vec![(token_id1.clone(), class_id1.clone())], tokens.tokens);
    let tokens = contract
        .all_tokens(
            deps.as_ref(),
            Some(TokenParams {
        ...
        .unwrap();
    assert_eq!(1, tokens.tokens.len());
    assert_eq!(vec![(token_id2.clone(), class_id2.clone())], tokens.tokens);
...
fn query_tokens_by_owner() {
    ...
    let expected = vec![
        (token_id1.clone(), class_id1.clone()),
        (token_id2.clone(), class_id2.clone()),
        (token_id3.clone(), class_id3.clone()),
    ];
    let tokens = contract.all_tokens(deps.as_ref(), None, None).unwrap();
    ...
    let tokens = contract
        .all_tokens(
            deps.as_ref(),
            Some(TokenParams {
                token_id: expected[1].0.clone(),
                class_id: expected[1].1.clone(),
            }),
            None,
        )
        .unwrap();
    ...
    // get by owner
    let by_ceres = vec![(token_id2, class_id2)];
    let by_demeter = vec![(token_id1, class_id1), (token_id3, class_id3)];
    // all tokens by owner
    let tokens = contract
    ...
    let tokens = contract
        .tokens(
            ...
            demeter,
            Some(TokenParams {
                token_id: by_demeter[0].0.clone(),
                class_id: by_demeter[0].1.clone(),
            }),
            Some(3),
        )
        .unwrap();
